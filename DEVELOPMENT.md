...
[ftpserverlib](https://github.com/fclairamb/ftpserverlib) speaks the protocol; what this repository adds is the single user `driver`, the certificate, and the summary.
`driver.AuthUser` returns an `afero.NewBasePathFs` rooted at the home directory, which is what keeps a client from reaching anything above it.
Keep it that way when changing the file system.
It comes back wrapped twice, by `pathHidingFs` and then by `loggingFs`, so an operation worth logging is added to `logfs.go` rather than to the library.

`Config.Prepare` is where a bad combination of flags has to be caught.
...
Add the section of the new version to `CHANGELOG.md` first, then write `RELEASE.md` for the release page: it holds the notes of the version being released only, while `CHANGELOG.md` keeps every version.
